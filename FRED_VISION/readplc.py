...
def read_bool_from_plc(client, db_number, byte_idx, bit_idx):
    if client is None:
        print("PLC not connected, skipping read operation.")
        return False
    try:
        data = client.db_read(db_number, byte_idx, 1)
        return get_bool(data, 0, bit_idx)
    except Exception as e:
        print(f"Failed to read from PLC: {e}")
        return False

try:
    while True:
        # Leer señal de activación desde el PLC
        trigger = read_bool_from_plc(plc, db_number=10, byte_idx=0, bit_idx=1)

        if trigger:
            print("Señal del PLC recibida. Ejecutando análisis de imagen...")
            ...
            print(f"Esperando señal del PLC, {datetime.now()}")
